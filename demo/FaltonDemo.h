...
void Demo8_init(ftPhysicsSystem *physicsSystem)
{
    ...
    const auto setMotorColliderGroup = [](ftCollider* collider) -> void {
        collider->group = 1;
    };
    ...
    body2->forEachCollider(setMotorColliderGroup);

    ftBody *body3 = CreateBall(physicsSystem, pivot1, 10, 0.5, 0.2, 0);
    body3->forEachCollider(setMotorColliderGroup);
    
    ftBody *body4 = CreateBall(physicsSystem, pivot2, 10, 0.5, 0.2, 0);
    body4->forEachCollider(setMotorColliderGroup);
...
// Merry Christmas
void Demo15_init(ftPhysicsSystem *physicsSystem)
{

    char *merryxmas =
        "#####O###O##OOOOO##OOOOO##OOOOO##O###O#####OOOOO##O###O##OOOOO##O##OOOOO##OOOOO##O###O####O####OOOOO#######"
        "#####OO#OO##O######O###O##O###O###O#O######O######O###O##O###O##O##O########O####OO#OO###O#O###O###########"
        "#####O#O#O##OOOO###OOOO###OOOO#####O#######O######OOOOO##OOOO###O##OOOOO####O####O#O#O##OOOOO##OOOOO#######"
        "#####O###O##O######OOOO###OOOO#####O#######O######O###O##OOOO###O######O####O####O###O##O###O######O#######"
        "#####O###O##OOOOO##O###O##O###O####O#######OOOOO##O###O##O###O##O##OOOOO####O####O###O##O###O##OOOOO#######";
    ...
    for (int i = 0; i < 107; ++i)
    {
        for (int j = 0; j < 5; ++j)
        {
            ...
            position.x = i;
            position.y = 5 - j;
            if (merryxmas[j * 107 + i] == 'O')
                CreateBall(physicsSystem, position, 1, 0.5, 0.2, 0.88);
        }
    }

    ftBody* bullet = CreateBall(physicsSystem, ftVector2(-10, 2.5), 1, 0.5, 0.2, 0.88);
    bullet->velocity = {10, 0};

    physicsSystem->m_gravity = {0, 0};
}

constexpr int NUM_DEMO = 15;
...
ftDemo demo[NUM_DEMO] = {
    ...
    {"MerryXmas", Demo15_init, DemoDefault_Update, DemoDefault_Cleanup}
};
}
#endif //FALTON_DEMO_H
