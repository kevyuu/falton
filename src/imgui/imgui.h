...
struct ImGuiStyle
{
    ...
    ImVec2      ItemInnerSpacing;           // Horizontal and vertical spacing between within m_elements of a composed widget (e.g. a slider and its label)
    ...
    bool        AntiAliasedShapes;          // Enable anti-aliasing on filled shapes (rounded rectangles, m_circles, etc.)
    float       CurveTessellationTol;       // Tessellation tolerance. Decrease for highly tessellated curves (higher quality, more m_polygons), increase to reduce quality.
...
// Usage:
//     ImGuiListClipper clipper(1000);  // we have 1000 m_elements, evenly spaced.
//     while (clipper.Step())
//         for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
//             ImGui::Text("line number %d", i);
// - Step 0: the clipper let you process the first element, regardless of it being visible or not, so we can measure the element height (step skipped if we passed a known height as second arg to constructor).
// - Step 1: the clipper infer height from first element, calculate the actual range of m_elements to display, and position the cursor before the first element.
...
// Draw command list
// This is the low-level list of m_polygons that ImGui functions are filling. At the end of the frame, all command lists are passed to your ImGuiIO::RenderDrawListFn function for rendering.
