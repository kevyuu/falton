...
/*
 ...
 - FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
   ...
   - I integrated ImGui in my engine and some m_elements are clipping or disappearing when I move windows around..
 ...
 Q: I integrated ImGui in my engine and some m_elements are clipping or disappearing when I move windows around..
 ...
 A: Yes. A primer on the use of labels/IDs in ImGui..
   ...
   - Use PushID() / PopID() to create scopes and avoid ID conflicts within the same Window.
     This is the most convenient way of distinguishing ID if you are iterating and creating many UI m_elements.
 ...
 A: Use the font atlas to pack them into a single texture:
    (Read extra_fonts/README.txt and the code in ImFontAtlas for more details.)
      ...
      m_config.GlyphExtraSpacing.x = 1.0f;
      io.Fonts->LoadFromFileTTF("myfontfile.ttf", size_pixels, &m_config);
      ...
      io.Fonts->LoadFromFileTTF("fontawesome-webfont.ttf", 16.0f, &m_config, ranges);
      io.Fonts->LoadFromFileTTF("myfontfile.ttf", size_pixels, NULL, &m_config, io.Fonts->GetGlyphRangesJapanese());
...
!- keyboard: tooltip & combo boxes are messing up / not honoring keyboard tabbing
 ...
 - style editor: have a more global HSV setter (e.g. alter hue on all m_elements). consider replacing active/hovered by offset in HSV space? (#438)
...
ImGuiStyle::ImGuiStyle()
{
    ...
    ItemInnerSpacing        = ImVec2(4,4);      // Horizontal and vertical spacing between within m_elements of a composed widget (e.g. a slider and its label)
    ...
    AntiAliasedShapes       = true;             // Enable anti-aliasing on filled shapes (rounded rectangles, m_circles, etc.)
    CurveTessellationTol    = 1.25f;            // Tessellation tolerance. Decrease for highly tessellated curves (higher quality, more m_polygons), increase to reduce quality.
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 1) // Step 1: the clipper infer height from first element, calculate the actual range of m_elements to display, and position the cursor before the first element.
    {
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        // Although we are active we don't prevent mouse from hovering other m_elements unless we are interacting right now with the widget.
