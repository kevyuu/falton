...
void ftDynamoJoint::warmStart(ftVector2* vArray __attribute__((unused)), real* wArray) {
    wArray[bodyIDB] += invMomentB * iAcc;
    wArray[bodyIDA] -= invMomentA * iAcc;
}

void ftDynamoJoint::solve(real dt, ftVector2 *vArray __attribute__((unused)), real *wArray) {
